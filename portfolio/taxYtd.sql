...
WITH net AS (
  -- 将月结单中的现金结余、个股市值、个基市值等按当月hkd/usd汇率统一折算为cny后求和,得到每月资产净值合计
  SELECT
  b.period_yyyymm,
  SUM(
    CASE
...
cash_dividend AS (
  -- 将交易流水中的出入金和派息及派息税费记录按当月hkd/usd汇率统一折算为cny后求和,得到每月出入金和净派息合计
  SELECT
...
monthly_data AS (
  -- 将上述2个子查询的结果按月合并,得到每月的cny资产净值、cny资产净值月度变化 、cny出入金合计、cny净派息合计
  SELECT
...
SELECT
  ...
  cny_net_diff,
  cny_cash_dividend,
  cny_net_diff - cny_cash_dividend AS cny_profit,
  CASE
  	WHEN MOD(period_yyyymm, 100) = 12 THEN
  	  SUM(cny_net_diff - cny_cash_dividend) OVER (
