...
def place(stations: List[int], distances: List[int], left, right) -> bool:
    if not distances:
        return True
    ...
    for d in relates:
        ...
        stations[right] = x
        found = place(stations, distances, left, right - 1)     # recurse to subsequents
    ...
    for d in relates:
        ...
        stations[left] = x
        found = place(stations, distances, left + 1, right)
        if found:
            return True
        else:
            print('backtracking ...')
...
def turnpike(distances: List[int], n_stat: int) -> List[int]:
    ...
    place(stations, distances, 1, n_stat - 2)
    return stations


def main():
    print(turnpike([1, 2, 2, 2, 3, 3, 3, 4, 5, 5, 5, 6, 7, 8, 10], 6))
